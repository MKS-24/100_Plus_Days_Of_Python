...
fifty_fifty = [
    "A) Earth\nB) Mars",  # Which planet is known as the Red Planet? (Correct: B) Mars)
    "C) Thomas Edison\nD) Benjamin Franklin",  # Who invented the light bulb? (Correct: C) Thomas Edison)
    "A) 5\nC) 7",  
    "B) 144\nC) 132",  # What is the square of 12? (Correct: B) 144)
    "A) 20\nD) 30",  # What is 15% of 200? (Correct: C) 25)
    "A) 20\nB) 30",  # If you subtract 20 from 100, then divide by 4, what is the result? (Correct: B) 30)
    "C) 1947\nD) 1948",  # When did Pakistan gain independence? (Correct: C) 1947)
    "A) Jasmine\nC) Sunflower",  # What is the national flower of Pakistan? (Correct: A) Jasmine)
    "A) Liaquat Ali Khan\nD) Zulfikar Ali Bhutto",  # Who was the first Prime Minister of Pakistan? (Correct: A) Liaquat Ali Khan)
    "B) H2O\nD) CH4",  # What is the chemical symbol for water? (Correct: B) H2O)
    "C) Heart\nD) Liver",  # Which organ in the human body is responsible for pumping blood? (Correct: C) Heart)
    "B) Oxygen\nD) Hydrogen",  # What gas do plants release during photosynthesis? (Correct: A) Carbon dioxide)
    "A) Aristotle\nC) Herodotus",  # Who is known as the “Father of History”? (Correct: D) Plato)
    "A) Roman Empire\nC) Qin Dynasty",  # Which empire built the Great Wall of China? (Correct: C) Qin Dynasty)
    "B) Brazil\nC) Italy"  # Which country has won the most FIFA World Cups? (Correct: A) Germany)
]
